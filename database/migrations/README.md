...
### `seed_questions.sql` ⭐ ОБЯЗАТЕЛЬНО
**Когда выполнять:** После `seed_competencies.sql`, для создания вопросов

**Что делает:**
- Создает примеры вопросов для компетенций:
  - React: вопросы по React Basics, Hooks, State Management
  - Go: вопросы по Go Basics, Concurrency
  - Общие Frontend: JavaScript/TypeScript, HTML/CSS
  - Общие Backend: RESTful API, SQL
- Каждая компетенция получает несколько вопросов разной сложности (1-5)
- Вопросы имеют expected_key_points и estimated_answer_time

**Важно:** Без этого скрипта генерация вопросов будет возвращать ошибку 404 "Question not found"
...
-- Выполните seed_questions.sql (ОБЯЗАТЕЛЬНО - создаст вопросы для компетенций)
...
Если вы получаете эту ошибку при выполнении SQL скриптов:

1. **Используйте Supabase Dashboard SQL Editor** (не через API клиент)
2. **Выполняйте скрипты по частям** - разбейте большой скрипт на меньшие
3. **Используйте исправленную версию** - `seed_questions_fixed.sql` использует функцию и более надежен
4. **Проверьте подключение:**
   ```sql
   SELECT 1 as test;
   ```

Подробная инструкция по решению проблем: [TROUBLESHOOTING.md](./TROUBLESHOOTING.md)
